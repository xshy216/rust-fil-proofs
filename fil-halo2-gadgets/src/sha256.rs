    ...
    pub fn configure(
        ...
        let s_u32_reorder_bits = meta.selector();
        {
            ...
            meta.create_gate("u32_reorder_bits", |meta| {
                ...
                for (bit, radix_pow) in le_bits.iter().zip(radix_pows.iter()) {
        ...
        layouter.assign_region(
            ...
            |mut region| {
                ...
                let u32s: [Option<u32>; 8] = match field.value() {
                    Some(field) => {
                        let mut u32s = [None; 8];
                        for (uint32, bytes) in u32s
                            .iter_mut()
                            .zip(field.to_repr().as_ref().chunks(4))
                        {
                            let bytes: [u8; 4] = bytes.try_into().unwrap();
                            *uint32 = Some(u32::from_le_bytes(bytes));
                        }
                        u32s
                    }
                    None => [None; 8],
                ...
                // Copy each `u32`, decompose the `u32` into little-endian bits, and assign the reordered
                // bits as a `u32`.
                for (uint32_index, uint32) in u32s.iter().enumerate() {
                    ...
                    let mut bit_index = 0;
                    for (offset, byte) in le_bits.chunks(8).enumerate() {
